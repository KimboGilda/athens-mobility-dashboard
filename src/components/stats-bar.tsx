...
const StatsBar = ({ stops, isDarkMap }: {
  ...
  const cardClass = isDarkMap
    ? 'bg-[#0d1422]/90 border border-[#1e2d44]'
    : 'bg-white/90 border border-gray-200'

  const labelClass = isDarkMap
    ? 'font-mono text-[9px] tracking-widest uppercase text-[#3a4a60] mb-1'
    : 'font-mono text-[9px] tracking-widest uppercase text-gray-400 mb-1'

  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000] flex gap-4">

      <div className={`${cardClass} rounded-lg px-4 py-3 min-w-[120px]`}>
        <p className={labelClass}>Metro Lines</p>
        <p className="font-mono text-[13px] font-bold text-[#2dd4a4]">{metroLinesStat}</p>
      </div>

      <div className={`${cardClass} rounded-lg px-4 py-3 min-w-[120px]`}>
        <p className={labelClass}>Metro Stations</p>
        <p className="font-mono text-[13px] font-bold text-[#2dd4a4]">{totalStations}</p>
      </div>

      <div className={`${cardClass} rounded-lg px-4 py-3 min-w-[120px]`}>
        <p className={labelClass}>Bus Stops</p>
